...
module.exports = function invocar(clave,funcion,registro,bPrctoFab,noPtosFab,res){
    ...
    var request = {
      chaincodeId: 'fabcar',
      fcn: 'changeCarOwner',
      args: ['CAR10', 'Rodobertro'],
      txId: tx_id
    };*/
    if (funcion=='redimePtos') {
        var request = {
          chaincodeId: 'eticket',
          fcn: funcion,
          args: clave,
      ...
      var request = {
          chaincodeId: 'eticket',
          fcn: funcion,
          args: [clave,registro,bPrctoFab.toString(),noPtosFab.toString()],
          txId: tx_id
        }
    }
      ...
      if (proposalResponses && proposalResponses[0].response &&
        ...
        if (funcion=='redimePtos') {
            console.error('Respuesta fue: ',JSON.stringify(proposalResponses[0].response.message));
            res.status(405).json(proposalResponses[0].response.message); 
          }
      }
      if (isProposalGood) {
